...
long address = 0;
_Bool isPlaying = 1;
_Bool samplePending = 0;



__attribute__((inline)) void pulse() {
    PORTCbits.RC0 = 1;
    PORTCbits.RC0 = 0;
}

_Bool fourCCeq(FourCC a, FourCC b) {
    for (char i=0; i<4; i++) {
        if (a[i] != b[i]) return 0;
    }
    return 1;
...
void openFile(long a) {
    ...
    {
        RiffTag riff;
        readBytes((char*)&riff, sizeof(riff));;
        if (!fourCCeq(riff.ckID, "RIFF")) error(RIFF_HEADER);
    }{
        FmtChunk fmt;
        readBytes((char*)&fmt, sizeof(fmt));;
        if (!fourCCeq(fmt.WAVEID, "WAVE")) error(WAVE_HEADER);
        if (!fourCCeq(fmt.fmtHeader.ckID, "fmt ")) error(FMT_HEADER);
        if (fmt.formatTag != 0x0001) error(CODEC);
        ...
        RiffTag data;
        readBytes((char*)&data, sizeof(data));;
        if (!fourCCeq(data.ckID, "data")) error(DATA_HEADER);
